...
public class TaskController {
    ...
    public ResponseEntity getTasksByDifficulty(@PathVariable("taskDifficulty") long taskDifficulty){
        try {
            System.out.println("###Loading taks by difficulty");
            return new ResponseEntity<>(service.getTasksByNumOfEmployees(taskDifficulty), null, HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseEntity<>(new ErrorResponse(0, "Error", "Something went wrong"), null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
